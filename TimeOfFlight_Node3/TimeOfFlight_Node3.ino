/* 
This program is an exact copy of the TimeOfFlight_Node1 created by Sri Mukunthraj, with the only change being in CAN_ID
This code will serve to collect and send data from the third Time of Flight sensor, and simulate a busy bus.
This sensor will be just a little lower in priority compared to the first and second sensor (determined by CAN ID), but will still be pretty high up there
...
VL53L0X sensor;  
static uint32_t const CAN_ID = 0x12;      // Setting the CAN ID of this node. This is also used for message prioritization, and can be changed as needed
